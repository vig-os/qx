...
enum GateMode {
    /// Build the gate from source at CI time (`cargo install --git`). Fine
    /// for development; not the audited artifact.
    #[default]
    Dev,
    /// Run the gate binary vendored under `.qx/gate/` (ADR-038 regulated
    /// default): pin-verified, repo-alone-verifiable, zero-network. Seed the
    /// binary with `qx gate vendor <version>`.
    Vendored,
}
...
enum Cmd {
    ...
    Init {
        ...
        #[arg(long)]
        force: bool,
        /// How the scaffolded repo runs its gate (ADR-038): `dev` builds from
        /// source; `vendored` runs a pin-verified binary carried in `.qx/gate/`.
        #[arg(long, value_enum, default_value_t = GateMode::Dev)]
        gate: GateMode,
    },
...
fn main() -> ExitCode {
    ...
    match cli.cmd {
        ...
        Cmd::Init { path, force, gate } => init_repo(&path, force, gate),
        Cmd::Promote {
            collection,
            key,
            field_type,
...
/// The `vendored` gate runner (ADR-038 regulated default): no toolchain, no
/// network — the gate is a pin-verified static musl binary carried in
/// `.qx/gate/`. Pin-verify-before-exec is done by SHELL (`sha256sum -c`), NOT
/// by running the binary to check itself. Seed the binary with
/// `qx gate vendor <version>`.
const CHECK_WORKFLOW_VENDORED: &str = r#"# Generated by `qx init --gate vendored` — the ADR-016/039 gate, run from
# the pin-verified static gate binary vendored in `.qx/gate/` (ADR-038).
# Repo-alone verifiable, zero-network, no toolchain. Seed/upgrade the binary
# with `qx gate vendor <version>` and commit `.qx/gate/`.
name: check
on:
  pull_request:
  push:
  ...
  pr-check:
    ...
    steps:
      - uses: actions/checkout@v4
        with:
          fetch-depth: 0
      # pin-verify-before-exec (ADR-034 §2): the vendored binary's sha256
      # must match its manifest BEFORE it runs. Done in shell so a tampered
      # binary is never executed (not `qx gate verify`, which would run it).
      - name: pin-verify the vendored gate
        run: |
          SHA=$(sed -n 's/^ *sha256 *= *"\(.*\)".*/\1/p' .qx/gate/manifest.toml)
          [ -n "$SHA" ] || { echo "::error::.qx/gate/manifest.toml has no sha256"; exit 1; }
          echo "$SHA  .qx/gate/qx" | sha256sum -c -
          chmod +x .qx/gate/qx
      - name: Validate against the contract
        run: .qx/gate/qx check --path . --base "origin/${{ github.base_ref || 'main' }}"
"#;

/// Manifest skeleton written by `qx init --gate vendored`. `qx gate vendor`
/// fills `version` + `sha256` and drops the binary + provenance beside it.
const GATE_MANIFEST_SKELETON: &str = r#"# The vendored gate (ADR-038 §1). Seed with `qx gate vendor <version>`.
knob = "vendored"
version = "UNSEEDED"
binary = "qx"
sha256 = "UNSEEDED"
attestation = "qx.intoto.jsonl"
source = "qx-src.tar.zst"
recipe = "flake.lock"
"#;

/// README dropped in `.qx/gate/` so the vendored-gate contract is obvious.
const GATE_DIR_README: &str = r#"# `.qx/gate/` — the vendored gate (ADR-038)

This directory carries the **audited gate binary this repo runs on every PR**,
so the gate is verifiable *and rebuildable* from the repo alone — no network,
no upstream release dependency at gate time.

Seed / upgrade it with:

```
qx gate vendor <version>   # e.g. qx gate vendor v0.14.0
```

which drops here: the static musl `qx` binary, its `sha256` (pinned in
`manifest.toml`), the sigstore attestation, the source tarball, and the Nix
recipe (`flake.lock`). CI pin-verifies the `sha256` before running the gate;
`qx gate verify --path .qx/gate` does the same locally.
"#;
...
fn init_repo(path: &Path, force: bool, gate: GateMode) -> ExitCode {
    ...
    match gate {
        GateMode::Dev => write_file!(".github/workflows/check.yml", CHECK_WORKFLOW),
        GateMode::Vendored => {
            write_file!(".github/workflows/check.yml", CHECK_WORKFLOW_VENDORED);
            // Manifest skeleton — `qx gate vendor <version>` fills version +
            // sha256 + drops the binary/provenance next to it.
            write_file!(".qx/gate/manifest.toml", GATE_MANIFEST_SKELETON);
            write_file!(".qx/gate/README.md", GATE_DIR_README);
        }
    }
...
mod tests {
    ...
    #[test]
    fn init_gate_vendored_scaffolds_the_pin_verified_runner() {
        let tmp = tempfile::tempdir().unwrap();
        let _ = init_repo(tmp.path(), false, GateMode::Vendored);
        let wf = std::fs::read_to_string(tmp.path().join(".github/workflows/check.yml"))
            .expect("vendored init wrote check.yml");
        assert!(wf.contains(".qx/gate/qx check"), "runs the vendored binary");
        assert!(wf.contains("pin-verify"), "pin-verifies before exec");
        assert!(!wf.contains("cargo install"), "no toolchain / no network");
        let man = std::fs::read_to_string(tmp.path().join(".qx/gate/manifest.toml")).unwrap();
        assert!(man.contains("knob = \"vendored\""), "{man}");
        assert!(tmp.path().join(".qx/gate/README.md").exists());
    }

    #[test]
    fn init_gate_dev_scaffolds_the_from_source_runner() {
        let tmp = tempfile::tempdir().unwrap();
