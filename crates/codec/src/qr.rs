...
//! swap — maintained, dual MIT/Apache, adds Micro QR M3 and rMQR;
//! resolves the `qrcode` 0.14 dormancy). Pinned via a `[patch.crates-
//! io]` to our fork's Annex-J optimal-segmentation branch (issue #211):
//! upstream's segmenter could pick a suboptimal mode split that
//! overran Micro QR capacity, falsely rejecting ~3% of 14-char ids;
//! the fork's single-mode-clamp + Annex-J fix makes **M3-L hold every
//! nano14 id** (verified 20k/20k). Version and EC level are contract
//! parameters (ADR-031 §8): [`encode_pinned`] takes them explicitly;
//! [`encode`] keeps the pre-contract defaults (Standard QR V1 / Micro
//! QR M4, both EC M) for the mm-native render path.
//!
